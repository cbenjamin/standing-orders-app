...
export const action = async ({ request, params }) => {
  ...
  if (intent === "search-customers") {
    const customers = await searchCustomers(admin, formData.get("q"));
    return { customers };
  }
  if (intent === "search-products") {
    const products = await searchProducts(admin, formData.get("q"));
    return { products };
  }
  ...
  await prisma.$transaction([
    prisma.standingOrderItem.deleteMany({ where: { standingOrderId: Number(params.id) } }),
    prisma.standingOrder.update({
      where: { id: Number(params.id) },
      data: {
        shopifyCustomerId, customerName, customerEmail, name,
        startDate, endDate, deliveryDay, closeDay, closeTime, sendReminder, sendCreationEmail,
  ...
  return redirect(`/app/standing-orders/${params.id}`);
};
...
export default function EditStandingOrder() {
  ...
  const handleCustomerSearch = useCallback(
    (val) => {
      ...
      if (val.length >= 2) customerFetcher.submit({ intent: "search-customers", q: val }, { method: "POST" });
    },
    [customerFetcher],
  );
  ...
  const handleProductSearch = useCallback(
    (val) => {
      ...
      if (val.length >= 2) productFetcher.submit({ intent: "search-products", q: val }, { method: "POST" });
    },
    [productFetcher],
  );
  ...
  return (
    <s-page heading={`Edit: ${order.name}`}>
      ...
      <Form method="POST">
        ...
        {actionData?.error && (
          <s-banner tone="critical" style={{ marginBottom: "1rem" }}>{actionData.error}</s-banner>
        )}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem", marginBottom: "0.75rem" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #e1e3e5" }}>
                  {["Product", "Price", "Min qty", ""].map((h) => <th key={h} style={{ padding: "0.4rem 0.6rem", textAlign: "left", color: "#6d7175", fontWeight: 500 }}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {items.map((item, idx) => (
                  <tr key={item.shopifyVariantId} style={{ borderBottom: "1px solid #f6f6f7" }}>
                    ...
                    <td style={{ padding: "0.5rem 0.6rem" }}>
                      <button type="button" onClick={() => removeItem(idx)} style={{ ...clearBtnStyle, color: "#d72c0d" }}>Remove</button>
                    </td>
                  </tr>
                ))}
              </tbody>
                ...
                {productResults.flatMap((p) =>
                  p.variants.map((v) => (
                    <DropdownItem key={v.id} onClick={() => addItem(p, v)}>
                      ...
                      {v.title !== "Default Title" && <span style={{ color: "#6d7175", marginLeft: "0.5rem" }}>— {v.title}</span>}
...
function DropdownItem({ onClick, children }) {
  return (
    <div onClick={onClick} style={{ padding: "0.5rem 0.75rem", cursor: "pointer", borderBottom: "1px solid #f6f6f7", fontSize: "0.875rem", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif" }} onMouseEnter={(e) => (e.currentTarget.style.background = "#f6f6f7")} onMouseLeave={(e) => (e.currentTarget.style.background = "")}>
      {children}
    </div>
  );
}
...
const inputStyle = { width: "100%", padding: "0.5rem 0.75rem", border: "1px solid #8c9196", borderRadius: 4, fontSize: "0.9375rem", background: "#fff", boxSizing: "border-box", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif" };
